...
static const u16 sBannedBackupSpecies[] =
{
	SPECIES_CHERRIM_SUN,
	SPECIES_SHAYMIN_SKY,
...
void DoFormChange(u8 bank, u16 species, bool8 ReloadType, bool8 ReloadStats, bool8 reloadAbility)
{
	...
	gActiveBattler = bank;
	mon = GetBankPartyData(bank);
    
	backup = mon->box.species;
	...
	BtlController_EmitSetMonData(0, REQUEST_FORM_CHANGE_BATTLE, 0, sizeof(struct BattlePokemon), &gBattleMons[bank]);
	...
	gStatuses3[bank] &= ~(STATUS3_INTIMIDATE_POKES | STATUS3_ILLUSION); //A Pokemon undergoing form change can't be hidden under Illusion
...
void SwitchOutFormsRevert(u8 bank)
{
	...
	u16 backupSpecies = mon->box.backupSpecies;
...
{
	return GetMiniorCoreFromPersonality(mon->box.personality);
}
...
void HandleFormChange(void)
{
	...
	mon->box.backupSpecies = GetMonData(mon, MON_DATA_SPECIES, NULL);
...
bool8 TryFormRevert(struct Pokemon* mon)
{
	...
	if (IsMinior(species))
	{
		mon->box.species = GetMiniorCoreSpecies(mon);
		CalculateMonStats(mon);
		return TRUE;
	}
	else if (IsGigantamaxSpecies(mon->box.backupSpecies) || CheckTableForSpecies(mon->box.backupSpecies, sBannedBackupSpecies)) //Forms the mon shouldn't revert to
	{
		mon->box.backupSpecies = SPECIES_NONE;
	}
	else if (mon->box.backupSpecies != SPECIES_NONE && mon->box.backupSpecies < NUM_SPECIES)
	{
		mon->box.species = mon->box.backupSpecies;
		mon->box.backupSpecies = SPECIES_NONE;
		oldHP = mon->hp;
		CalculateMonStats(mon);
	...
	else if (mon->box.species == SPECIES_SHAYMIN_SKY)
	{
		if (IsNightTime())
		{
			mon->box.species = SPECIES_SHAYMIN; //Shaymin reverts to normal form at night
			CalculateMonStats(mon);
			return TRUE;
		}
	}
	...
	else if (mon->box.species == SPECIES_KELDEO)
	{
		for (i = 0; i < MAX_MON_MOVES; ++i)
		{
			if (mon->box.moves[i] == MOVE_SECRETSWORD)
				break;
		}

		if (i != MAX_MON_MOVES) //Keldeo knows Secret Sword
		{
			mon->box.species = SPECIES_KELDEO_RESOLUTE;
			CalculateMonStats(mon);
			return TRUE;
		}
	}
	else if (mon->box.species == SPECIES_KELDEO_RESOLUTE)
	{
		for (i = 0; i < MAX_MON_MOVES; ++i)
		{
			if (mon->box.moves[i] == MOVE_SECRETSWORD)
		...
		{
			mon->box.species = SPECIES_KELDEO;
			CalculateMonStats(mon);
			return TRUE;
		}
	}
...
void HoldItemFormChange(struct Pokemon* mon, u16 item) // need todo
{
	...
	switch(species) 
	{
		...
		case SPECIES_ARCEUS_FAIRY:
			...
			if (ability == ABILITY_MULTITYPE) //Only transform if set with proper ability
			{
				...
				|| (itemEffect == HOLD_EFFECT_Z_CRYSTAL && !IsSpecialZCrystal(item)))
