...
static const u32 Mega_IndicatorTiles[] = INCBIN_U32("graphics/battle_interface/mega/Mega_Indicator.4bpp.lz");
static const u16 Mega_IndicatorPal[] = INCBIN_U16("graphics/battle_interface/mega/Mega_Indicator.gbapal");

static const u32 Alpha_IndicatorTiles[] = INCBIN_U32("graphics/battle_interface/mega/Alpha_Indicator.4bpp.lz");
static const u32 Omega_IndicatorTiles[] = INCBIN_U32("graphics/battle_interface/mega/Omega_Indicator.4bpp.lz");
static const u32 Ultra_IndicatorTiles[] = INCBIN_U32("graphics/battle_interface/mega/Ultra_Indicator.4bpp.lz");

static const u32 Mega_TriggerTiles[] = INCBIN_U32("graphics/battle_interface/mega/Mega_Trigger.4bpp.lz");
static const u16 Mega_TriggerPal[] = INCBIN_U16("graphics/battle_interface/mega/Mega_Trigger.gbapal");

static const u32 Ultra_TriggerTiles[] = INCBIN_U32("graphics/battle_interface/mega/Ultra_Trigger.4bpp.lz");
static const u32 Z_Move_TriggerTiles[] = INCBIN_U32("graphics/battle_interface/mega/Z_Move_Trigger.4bpp.lz");
static const u16 Z_Move_TriggerPal[] = INCBIN_U16("graphics/battle_interface/mega/Z_Move_Trigger.gbapal");

static const u32 Dynamax_IndicatorTiles[] = INCBIN_U32("graphics/battle_interface/mega/Dynamax_Indicator.4bpp.lz");
static const u32 Dynamax_Trigger_WorkingTiles[] = INCBIN_U32("graphics/battle_interface/mega/Dynamax_Trigger_Working.4bpp.lz");

static const u16 Dynamax_TriggerPal[] = INCBIN_U16("graphics/battle_interface/mega/Dynamax_Trigger.gbapal");

static const u32 Raid_ShieldTiles[] = INCBIN_U32("graphics/battle_interface/mega/Raid_Shield.4bpp.lz");
...
static void SpriteCB_MegaTrigger(struct Sprite* self)
{
	...
	{
		if (!CanMegaEvolve(TRIGGER_BANK, FALSE) || gBattleSpritesDataPtr->battlerData[TRIGGER_BANK].transformSpecies)
			self->invisible = TRUE;
		else
	...
	{
		if (!CanMegaEvolve(TRIGGER_BANK, TRUE) || gBattleSpritesDataPtr->battlerData[TRIGGER_BANK].transformSpecies)
			self->invisible = TRUE;
		else
			self->invisible = FALSE;
	}
	...
	if (gBattlerControllerFuncs[TRIGGER_BANK] == HandleInputChooseMove
	...
	else if (gBattlerControllerFuncs[TRIGGER_BANK] != PlayerHandleChooseMove //PlayerHandleChooseMove
		  && gBattlerControllerFuncs[TRIGGER_BANK] != HandleChooseMoveAfterDma3) //HandleChooseMoveAfterDma3
	{
		if (self->data[3] < 32)
			self->data[3] += 2;
		else
	...
	moveInfo = (struct ChooseMoveStruct*) (&gBattleBufferA[TRIGGER_BANK][4]);
	if (TAG == GFX_TAG_MEGA_TRIGGER)
	{
		...
		if (evo->unknown != MEGA_VARIANT_ULTRA_BURST)
		{
			...
			{
				if (gBattleStruct->megaData.chosen[TRIGGER_BANK])
	...
	else //TAG == GFX_TAG_ULTRA_TRIGGER
	{
		...
		if (evo->unknown == MEGA_VARIANT_ULTRA_BURST)
		{
			...
			{
				if (gBattleStruct->ultraData.chosen[TRIGGER_BANK])
					PALETTE_STATE = MegaTriggerLightUp;
				else
					PALETTE_STATE = MegaTriggerNormalColour;
			}
	...
	{
		u16* pal = &gPlttBufferFaded[PLTT_BUFFER_SIZE / 2 + IndexOfSpritePaletteTag(PAL_TAG) * 16];
		u8 i;
...
static void SpriteCB_MegaIndicator(struct Sprite* self)
{
	...
	healthbox = GetHealthboxSprite(INDICATOR_BANK);
	...
	if (y)
	{
...

        {
            ...
            if (sineActive)
            {
                ...
                self->y2 = Sin(ping->data[0], ping->data[2]);
                return;
            }
            else if (shaker->data[1] == hbid && gShakerData[0])
            {
                self->y2 = (shaker->data[2] & 1) ? -1 : 1;
                return;
            }
        }
...
static void SpriteCB_ZTrigger(struct Sprite* self)
{
	...
	if (PALETTE_STATE != self->data[2])
	{
		u16* pal = &gPlttBufferFaded[PLTT_BUFFER_SIZE / 2 + IndexOfSpritePaletteTag(GFX_TAG_Z_TRIGGER) * 16];
		u8 i;
...
static void SpriteCB_DynamaxTrigger(struct Sprite* self)
{
	...
	if (PALETTE_STATE != self->data[2])
	{
		u16* pal = &gPlttBufferFaded[PLTT_BUFFER_SIZE / 2 + IndexOfSpritePaletteTag(GFX_TAG_DYNAMAX_TRIGGER) * 16];
		u8 i;
...
static void SpriteCB_RaidShield(struct Sprite* sprite)
{
	...
	sprite->x = healthbox->x + 50;
	sprite->y = healthbox->y + 12;
	...
	if (state == 2 || state == 0xFF)
	{
		...
		for (bank = 0; bank < gBattlersCount; ++bank)
		{
			...
			if (IsMega(bank))
			{
				...
				gBattleStruct->megaIndicatorObjIds[bank] = spriteId + 1;
			}
			...
			else if (IsDynamaxed(bank))
			{
				...
				gBattleStruct->megaIndicatorObjIds[bank] = 0;
...
{
	if (!gBattleStruct->megaIndicatorObjIds[gActiveBattler] && gActiveBattler < gBattlersCount)
	{
...
{
	if (gBattleStruct->megaIndicatorObjIds[bank] > 0)
	{
		DestroySprite(&gSprites[gBattleStruct->megaIndicatorObjIds[bank] - 1]);
		gBattleStruct->megaIndicatorObjIds[bank] = 0;
	}
}
...
{
	u8 i;
	++gBattleStruct->dynamaxData.shieldsDestroyed;

	for (i = 0; i < MAX_NUM_RAID_SHIELDS; ++i)
	{
		if (gBattleStruct->dynamaxData.shieldSpriteIds[i] == 0)
			break; //No more sprites

		if (gBattleStruct->dynamaxData.shieldCount - i - gBattleStruct->dynamaxData.shieldsDestroyed <= 0)
		{
			struct Sprite* sprite = &gSprites[gBattleStruct->dynamaxData.shieldSpriteIds[i] - 1];
