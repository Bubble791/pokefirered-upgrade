...
static void DoDnsLightning()
{
    ...
    for (i = 0; i < sizeof(gLightingColours)/sizeof(gLightingColours[0]); i++)
    {
        ...
        if (gSaveBlock1Ptr->location.mapGroup == gLightingColours[i].group && gSaveBlock1Ptr->location.mapNum == gLightingColours[i].mapnum)
        {
...
static bool8 IsLightActive()
{
    if (gClock.hour >= LIGHTNING_START_HOUR || gClock.hour < LIGHTNING_END_HOUR)
        return TRUE;
    return FALSE;
}
...
bool8 IsNight(void)
{
    ...
    if(GetDnsTimeLapse(hour) == TIME_NIGHT || GetDnsTimeLapse(hour) == TIME_MIDNIGHT)
        return TRUE;
    else
        return FALSE;
}
...
{
	return gClock.hour >= MIDNIGHT_END_HOUR && gClock.hour < NIGHTFALL_END_HOUR;
}

bool8 IsOnlyDayTime(void)
{
	return gClock.hour >= DAWN_END_HOUR && gClock.hour < SUNSET_END_HOUR;
}

bool8 IsNightTime(void)
{
	return gClock.hour >= NIGHTFALL_END_HOUR || gClock.hour < MIDNIGHT_END_HOUR;
}

bool8 IsMorning(void)
{
	return gClock.hour >= MIDNIGHT_END_HOUR && gClock.hour < DAWN_END_HOUR;
}

bool8 IsEvening(void)
{
	return gClock.hour >= SUNSET_END_HOUR && gClock.hour < NIGHTFALL_END_HOUR;
}
